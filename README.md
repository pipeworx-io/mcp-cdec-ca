...
`tools/list` at `https://gateway.pipeworx.io/cdec-ca/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/station_info \
  -H 'Content-Type: application/json' \
  -d '{"station_id":"SHA"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/station_info`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
